...
export class BiodataRequestRepository {
    async create(dto: CreateRequestDto) {
        const res = await db.query(
            `INSERT INTO bwpc_biodata_update_requests
    ...
    async findAllRequest() {
        const res = await db.query(`SELECT * FROM bwpc_biodata_update_requests`);
        return res.rows;
    }

    async findById(id: number) {
        const res = await db.query(`SELECT * FROM bwpc_biodata_update_requests WHERE id = $1`, [id]);
        return res.rows[0];
    }

    async findByClientId(client_id: number) {
        const res = await db.query(`SELECT * FROM bwpc_biodata_update_requests WHERE client_id = $1`, [client_id]);
        return res.rows;
    }

    async updateStatus(id: number, status: string) {
        const res = await db.query(
            `UPDATE bwpc_biodata_update_requests SET status = $1 WHERE id = $2 RETURNING *`,
            [status, id],
        );
        return res.rows[0];
    }
    ...
    async updateAprroved(id: number) {
        ...
        await db.query(
            `UPDATE bwpc_clients SET company_name = $1, contact_person = $2, phone = $3, address = $4 WHERE id = $5`,
            [
